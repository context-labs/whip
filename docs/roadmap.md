...
- [x] Interactive start-with-prompt: `whip up <words...>` joins argv after `up` and submits it as the TUI's first turn (claude's positional-prompt UX) — Init-kickoff msg so the turn starts only once `m.prog` exists. The folder-trust gate moves *into* the TUI (inline namePrompt) when no terminal is available pre-launch, holding the prompt until approved, exiting on decline. Plans: [`.ai-docs/plans/whip-up/`](../.ai-docs/plans/whip-up/README.md), [`.ai-docs/plans/whip-up-in-tui-trust/`](../.ai-docs/plans/whip-up-in-tui-trust/README.md)
