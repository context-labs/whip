...
try {
  for (const [name, engine] of Object.entries({chromium, firefox})) {
    ...
    try {
      ...
      await page.getByRole('button', {name: 'Claude Code controls'}).click();
      await page.waitForFunction(() => document.documentElement.dataset.theme === 'claude-code');
      const track = page.locator('[data-whip-slider-track]');
      const indicator = page.locator('[data-whip-slider-indicator]');
      const trackColor = await track.evaluate(el => getComputedStyle(el).backgroundColor);
      const trackHex = '#' + trackColor.match(/[\d.]+/g).slice(0, 3).map(value => Math.round(Number(value)).toString(16).padStart(2, '0')).join('');
      const panelColor = await page.evaluate(() => document.documentElement.style.getPropertyValue('--whip-panel'));
      if (contrastRatio(trackHex, panelColor) < 3) throw new Error(`${name}: interactive track is not distinguishable from its panel`);
      const density = page.getByRole('slider', {name: 'Tool density'});
      await density.focus(); await density.press('ArrowRight');
      if (await density.getAttribute('aria-valuetext') !== 'Comfortable') throw new Error('Slider named value/keyboard failed under CSP');
      const trackBounds = await track.boundingBox();
      const midpoint = await indicator.boundingBox();
      if (trackBounds.height < 4 || midpoint.height !== trackBounds.height || Math.abs(midpoint.width - trackBounds.width / 2) > 1) throw new Error(`${name}: slider midpoint geometry is incorrect`);
      await density.press('End');
      const maximum = await indicator.boundingBox();
      if (Math.abs(maximum.width - trackBounds.width) > 1) throw new Error(`${name}: slider full-range indicator is incorrect`);
      const exactCode = await code.textContent();
      await page.getByRole('button', {name: 'Increase readability'}).click();
      await page.waitForFunction(() => document.documentElement.dataset.contrast === 'more');
      await page.waitForFunction(() => getComputedStyle(document.querySelector('[aria-label="Starlark example"]')).fontSize === '24px');
      const display = await code.evaluate(el => ({size: getComputedStyle(el).fontSize, wrap: getComputedStyle(el).whiteSpace, family: getComputedStyle(el).fontFamily}));
      if (display.size !== '24px' || display.wrap !== 'pre-wrap' || display.family.includes('JetBrains')) throw new Error(`${name} display variables did not apply under CSP: ${JSON.stringify(display)}`);
      if (await code.textContent() !== exactCode) throw new Error('Wrapping changed source text');
      await page.getByRole('button', {name: 'Reset display'}).click();
      await page.waitForFunction(() => document.documentElement.dataset.contrast === 'standard');
