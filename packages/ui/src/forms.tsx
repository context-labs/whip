...
export function Switch({label, hideLabel = false, description, xstyle, className, style, ...props}: BaseSwitch.Root.Props & Styled & {label?: ReactNode; hideLabel?: boolean; description?: string}) {
  const id = useId();
  return <label {...stylex.props(styles.inline, styles.label, styles.checkLabel)}><BaseSwitch.Root {...props} aria-describedby={[props['aria-describedby'], description ? id : undefined].filter(Boolean).join(' ') || undefined} style={state => mergeProps(stylex.props(xstyle), {style: typeof style === 'function' ? style(state) : style}).style} className={state => mergeProps(stylex.props(styles.control, styles.switch, state.checked && styles.switchOn, xstyle), {className: typeof className === 'function' ? className(state) : className}).className}><BaseSwitch.Thumb className={state => stylex.props(styles.switchThumb, state.checked && styles.switchThumbOn).className}/></BaseSwitch.Root>{(label || description) && <span {...stylex.props(hideLabel && styles.visuallyHidden)}>{label}{description && <span id={id} {...stylex.props(styles.description, styles.optionDescription)}>{description}</span>}</span>}</label>;
}
export function RadioGroup({value, onValueChange, options, label, disabled, variant = 'default'}: {value?: string; onValueChange?: (value: string) => void; options: (Option & {trailing?: ReactNode})[]; label: string; disabled?: boolean; variant?: 'default' | 'cards'}) {
  const id = useId();
  const cards = variant === 'cards';
  return <BaseRadioGroup value={value} onValueChange={value => onValueChange?.(value as string)} aria-label={label} aria-labelledby={undefined} disabled={disabled} {...stylex.props(styles.stack, cards && styles.radioCards)}>{options.map((option, index) => <label key={option.value} {...stylex.props(styles.inline, styles.label, styles.checkLabel, cards && styles.radioCard, cards && value === option.value && styles.radioCardSelected)}>
    {cards && <span aria-hidden {...stylex.props(styles.radioCardIcon)}>{option.icon}</span>}
    <span {...stylex.props(cards && styles.radioCardBody)}><span id={`${id}-${index}-label`}>{option.label}</span>{option.description && <span id={`${id}-${index}`} {...stylex.props(styles.description, styles.optionDescription, cards && styles.radioCardDescription)}>{option.description}</span>}</span>
    <span {...stylex.props(cards && styles.radioCardEnd, !cards && styles.radioFirst)}>
      {cards && option.trailing ? <span {...stylex.props(styles.description)}>{option.trailing}</span> : null}
      <BaseRadio.Root value={option.value} disabled={option.disabled} aria-labelledby={`${id}-${index}-label`} aria-describedby={option.description ? `${id}-${index}` : undefined} className={state => stylex.props(styles.control, styles.checkbox, styles.radio, cards && styles.radioCardControl, state.checked && styles.checked, cards && !!option.trailing && styles.visuallyHidden).className}><BaseRadio.Indicator><Check size={12}/></BaseRadio.Indicator></BaseRadio.Root>
    </span>
