...
export const styles = stylex.create({
  ...
  active: { backgroundColor: { default: colors.background, ':hover': colors.background }, borderColor: colors.border },
  tab: { display: 'flex', alignItems: 'center', gap: 8, flex: '1 1 auto', minWidth: 0, height: '100%', paddingInlineStart: scale.space3, paddingInlineEnd: scale.space1, borderWidth: 0, borderStyle: 'none', backgroundColor: 'transparent', color: surface.secondaryText, fontFamily: typography.sans, fontSize: 13, fontWeight: 500, lineHeight: '20px', textDecoration: 'none', textAlign: 'start', whiteSpace: 'nowrap', cursor: 'pointer', userSelect: 'none', borderRadius: scale.radiusControl, outline: { default: 'none', ':focus-visible': `2px solid ${colors.borderFocus}` }, outlineOffset: -2 },
