...
export function CopyButton({text, copy, onError}: {text: string; copy?: (text: string) => Promise<void>; onError?: (error: unknown) => void}) {
  const [copied, setCopied] = useState<string | null>(null);
  const [error, setError] = useState(false);
  return <IconButton label={error ? 'Could not copy. Try again.' : copied === text ? 'Copied' : 'Copy'} variant="ghost" onBlur={() => {setCopied(null); setError(false);}} onClick={() => {void Promise.resolve().then(() => (copy ?? (value => navigator.clipboard.writeText(value)))(text)).then(() => {setCopied(text); setError(false);}, error => {setError(true); onError?.(error);});}}>{copied === text ? <Check size={14}/> : <Copy size={14}/>}</IconButton>;
}
