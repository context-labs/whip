...
export interface DialogProps extends Styled {open: boolean; onOpenChange: (open: boolean) => void; title: ReactNode; description?: ReactNode; children?: ReactNode; footer?: ReactNode; closeLabel?: string; initialFocus?: BaseDialog.Popup.Props['initialFocus']}
export function Dialog({open, onOpenChange, title, description, children, footer, closeLabel = 'Close', initialFocus, xstyle}: DialogProps) {
  return <BaseDialog.Root open={open} onOpenChange={onOpenChange}><BaseDialog.Portal><BaseDialog.Backdrop {...stylex.props(styles.backdrop)}/><BaseDialog.Popup initialFocus={initialFocus} {...stylex.props(styles.dialog, xstyle)}><div {...stylex.props(styles.dialogHeader)}><div><BaseDialog.Title {...stylex.props(styles.dialogTitle)}>{title}</BaseDialog.Title>{description && <BaseDialog.Description {...stylex.props(styles.dialogDescription)}>{description}</BaseDialog.Description>}</div><BaseDialog.Close render={<Button aria-label={closeLabel} variant="ghost" xstyle={styles.icon}><X size={16}/></Button>}/></div><div {...stylex.props(styles.stack)}>{children}</div>{footer && <div {...stylex.props(styles.footer)}>{footer}</div>}</BaseDialog.Popup></BaseDialog.Portal></BaseDialog.Root>;
}
...
export function Popover({trigger, title, children, open, onOpenChange}: {trigger: ReactElement; title?: ReactNode; children: ReactNode; open?: boolean; onOpenChange?: (value: boolean) => void}) {
  return <BasePopover.Root open={open} onOpenChange={onOpenChange}><BasePopover.Trigger render={trigger}/><BasePopover.Portal><BasePopover.Positioner sideOffset={8} {...stylex.props(styles.positioner)}><BasePopover.Popup {...stylex.props(styles.popup)}>{title && <BasePopover.Title {...stylex.props(styles.label)}>{title}</BasePopover.Title>}{children}</BasePopover.Popup></BasePopover.Positioner></BasePopover.Portal></BasePopover.Root>;
}
