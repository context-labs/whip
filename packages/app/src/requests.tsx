...
function QuestionRequest({
  ...
  async function submit(answerDrafts = drafts) {
      ...
      await runtime.run(
        question.questions?.length
          ? session.answerQuestions(
              ...
              answerDrafts.map((item) =>
                item.skipped ? null : { answer: draftAnswer(item) },
              ),
            )
          : session.answerQuestion(question.question_id, draftAnswer(answerDrafts[0]), answerDrafts[0].skipped),
        'Answer question',
      );
      restoreFocus();
    } catch {
        ...
        <div {...stylex.props(questionStyles.footer)}>
          ...
          <label {...stylex.props(questionStyles.custom)}>
            ...
            <Input
              ...
              value={draft.text}
              onChange={(event) => patchDraft({ text: event.target.value, skipped: false })}
          ...
          <Button
            ...
            onClick={() => {
              if (questions.length === 1) void dismissAll();
                ...
                const next = drafts.map((item, currentIndex) => currentIndex === index
