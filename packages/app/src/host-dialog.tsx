...
export function LocalRuntimePanel({ api, hostState, disabled, onBusyChange, onboarding = false, onConnect, progress, connectionError, inspection, inspectOnMount = true }: {
  ...
  inspection?: LocalRuntimeStatus; inspectOnMount?: boolean;
}) {
  const mounted = useRef(true);
...
We’ll install it with the recommended settings.`
  ...
  return <section aria-label="This Mac runtime" {...stylex.props(layout.column, onboarding && styles.setup)}>
    <div {...stylex.props(onboarding && styles.setupHeader)}>
      {onboarding && <h1 {...stylex.props(styles.setupTitle)}>{status?.state === 'missing' && status.repairRequired ? 'Repair Whip on this Mac' : 'Set up Whip on this Mac'}</h1>}
    ...
    {onboarding ? <>
      {installing || (status?.state === 'missing' && status.canInstall && !connecting && !unavailable) ? <div {...stylex.props(layout.row)}>
        <Button variant="primary" disabled={busy} loading={installing} onClick={() => void run(api.installDefault ? 'installDefault' : 'install')}>
          {installing ? 'Setting up…' : error ? 'Retry setup' : status?.repairRequired ? 'Repair installation' : 'Set up this Mac'}
        </Button>
