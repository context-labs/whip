...
import { Badge, Button, Dialog, Field, Input } from '@whip/ui';
...
const logoIDs: Record<string, string> = { 'inference-net': 'inference', openrouter: 'openrouter', 'openai-codex': 'openai', openai: 'openai' };
...
function ProviderLogo({ id }: { id: string }) {
  const name = logoIDs[id];
  return name ? <svg width="20" height="20" viewBox="0 0 40 40" aria-hidden="true" {...stylex.props(styles.logo)}>
    <use href={`${logos}#${name}`} />
  </svg> : <Plug size={20} aria-hidden="true" {...stylex.props(styles.logo)} />;
}
...
export function ProviderConnections({ client, enabled }: { client: WhipClient; enabled: boolean }) {
  ...
  return <>
    ...
    {unavailableDefault && <div role="status" {...stylex.props(layout.notice, styles.intro)}>
      ...
      <Button variant="ghost" onClick={() => document.getElementById('default_provider')?.querySelector('input')?.focus()}>Change default provider</Button>
    </div>}
    <ConfigurationSettings client={client} enabled={enabled} category="providers" />
...
function ProviderConnectionDialog({ client, entry, enabled, revision, hostName, flows, refresh, refreshFlows, close }: {
  ...
  return <Dialog open title={entry.name} description={`Provider connection on ${hostName}.`} onOpenChange={open => { if (!open) requestClose(); }}>
    <div {...stylex.props(styles.nameLine)}><ProviderLogo id={entry.id} /><Badge>{sourceLabel(entry) || 'Not connected'}</Badge></div>
    {stateLabel(entry) && <p role="status">{stateLabel(entry)}</p>}
    {entry.status.email && <p>{entry.status.email}{entry.status.plan ? ` · ${entry.status.plan}` : ''}</p>}
    {entry.status.project_name && <p>Project: {entry.status.project_name}</p>}
    ...
    {entry.status.warnings?.map((warning, index) => <p key={index} role="alert">{warning}</p>)}
    {entry.methods?.includes('login') && !activeFlow && <Button disabled={!enabled || busy} onClick={() => void action(async signal => {
    ...
    {entry.methods?.includes('api_key') && !showKey && <Button variant="secondary" disabled={!enabled || busy} onClick={() => setShowKey(true)}>{entry.status.available ? 'Replace API key' : 'Use an API key'}</Button>}
    {entry.methods?.includes('environment') && <Button variant="secondary" disabled={!enabled || busy} onClick={() => void action(async signal => {
    ...
    {showKey && <form onSubmit={event => {
      ...
      <Button type="submit" disabled={!enabled || busy || !key.trim()}>{busy ? 'Connecting…' : 'Connect'}</Button>
    </form>}
    {(entry.status.available || entry.status.configured || entry.status.disabled) && <Button variant="ghost" disabled={!enabled || busy} onClick={() => void action(async signal => {
    ...
    {entry.id === 'inference-net' && entry.status.key_source === 'machine' && entry.status.email && <Button variant="ghost" disabled={!enabled || busy} onClick={() => void action(async signal => {
    ...
    {canDisconnect && <Button variant="ghost" disabled={!enabled || busy} onClick={() => void action(async signal => {
    ...
    {discard && <div {...stylex.props(layout.notice, layout.column)}><p>Discard the unsaved API key?</p><div {...stylex.props(layout.row)}>
      <Button variant="secondary" onClick={() => setDiscard(false)}>Keep editing</Button><Button onClick={() => { setKey(''); close(); }}>Discard key</Button>
    </div></div>}
  </Dialog>;
}
...
const styles = stylex.create({
  ...
  identity: { display: 'flex', alignItems: 'center', gap: scale.space3, minWidth: 0, flex: '1 1 220px' },
  ...
  logo: { flexShrink: 0, color: colors.foreground, width: 20, height: 20 },
});
