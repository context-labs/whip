...
import { Collapsible, Field, Input } from '@whip/ui';
import * as stylex from '@stylexjs/stylex';
import type { ConnectionTarget } from './connections';
import { layout } from './styles';

export function SSHFields({ target, busy, onChange, children, hostRef }: { children?: ReactNode; hostRef?: RefObject<HTMLInputElement | null>; target: Extract<ConnectionTarget, { kind: 'ssh' }>; busy: boolean; onChange(target: Extract<ConnectionTarget, { kind: 'ssh' }>): void }) {
  ...
  return <>
        <Field label="SSH host or alias"><Input ref={hostRef} value={target.host ?? ''} required autoFocus disabled={busy} placeholder="my-server"
        ...
        {children}
        <Collapsible title="Advanced SSH options"><div {...stylex.props(layout.column)}>
