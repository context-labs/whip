...
it('refuses unknown hosts without trusting endpoint hints in a link', async () => {
  ...
  expect(f.runtime.reportWorkspace).toHaveBeenCalledWith(expect.objectContaining({ message: expect.stringContaining('unknown execution host') }));
});
it('keeps failed connection reporting scoped to its host record', async () => {
  ...
  await f.navigator.open('/h/remote/s/root');
  expect(f.navigate).not.toHaveBeenCalled(); expect(f.runtime.reportWorkspace).not.toHaveBeenCalled();
});
it.each(['resolve', 'reject'] as const)('does not override manual navigation when pending setup settles with %s', async outcome => {
  ...
  await opened; expect(f.navigate).not.toHaveBeenCalled(); expect(f.runtime.reportWorkspace).not.toHaveBeenCalled();
});
it('lets a newer native link supersede a pending navigation', async () => {
  ...
  expect(f.navigate).toHaveBeenCalledExactlyOnceWith('/h/mac/s/current'); expect(f.runtime.reportWorkspace).not.toHaveBeenCalled();
});
...
it('cancels pending navigation on disposal and ignores later native links', async () => {
  ...
  expect(f.connect).toHaveBeenCalledOnce(); expect(f.navigate).not.toHaveBeenCalled(); expect(f.runtime.reportWorkspace).not.toHaveBeenCalled();
});
for (const path of ['https://elsewhere.example/h/mac/s/root', '//elsewhere.example/h/mac/s/root', '/settings', '/h/mac/s/root#fragment', '/h/mac/s/']) {
  it(`rejects a non-session native route: ${path}`, async () => {
    ...
    expect(f.connect).not.toHaveBeenCalled(); expect(f.navigate).not.toHaveBeenCalled();
    expect(f.runtime.reportWorkspace).toHaveBeenCalledWith(expect.objectContaining({ message: 'Invalid session link' }));
  });
}
