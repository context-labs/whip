...
function observer(desktop = true) {
  ...
  const state = { preferences: { attentionAnnouncements: false, desktopNotifications: true }, hosts: [{ id: 'local', name: 'This Mac', client, runtimeId: 'runtime', state: 'connected' }] };
  const runtime = { platform: { notify, setNotificationsEnabled }, queries, subscribe: () => () => {}, getSnapshot: () => state,
    connections: { isAttached: (candidate: WhipClient) => candidate === client }, tabs: { preferred: () => undefined, subscribe: () => () => {}, getSnapshot: () => null }, report: vi.fn() } as unknown as AppRuntime;
  const mounted = render(<StrictMode><RuntimeContext.Provider value={runtime}><UIProvider><QueryClientProvider client={queries}>
    {desktop ? <DesktopAttention client={client} /> : <Attention />}
