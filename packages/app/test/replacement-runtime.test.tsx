...
it('preserves saved SSH identity by default and requires an explicit replacement choice', async () => {
  ...
  fireEvent.click(screen.getByRole('button', { name: /Save changes|Connect/ }));
...
it('does not close a reopened form when an old native request finishes', async () => {
  ...
  fireEvent.click(screen.getByRole('button', { name: /Save changes|Connect/ })); await waitFor(() => expect(f.save).toHaveBeenCalled()); f.reopen();
