...
function fixture() {
  ...
  const application = (open = true) => <RuntimeContext.Provider value={runtime}><UIProvider><HostDialog open={open} onOpenChange={onOpenChange} onSaved={onSaved} /></UIProvider></RuntimeContext.Provider>;
  const view = render(application());
  const edit = (name: string) => {
    const identity = within(screen.getByLabelText('Saved execution hosts')).getByText(name).closest('div')!.parentElement!;
...
it('preserves saved SSH identity by default and requires an explicit replacement choice', async () => {
  ...
  await waitFor(() => expect(f.save).toHaveBeenCalledExactlyOnceWith(f.ssh, false));
...
it('passes explicit replacement consent for the exact saved profile', async () => {
  ...
  fireEvent.click(screen.getByRole('button', { name: 'Save and connect' }));
  await waitFor(() => expect(f.save).toHaveBeenCalledExactlyOnceWith(f.ssh, true)); f.dispose();
});
it('supports managed-local replacement through the same host manager', async () => {
  const f = fixture(); f.edit('This Mac');
  fireEvent.click(screen.getByRole('button', { name: 'Save and connect' }));
  await waitFor(() => expect(f.save).toHaveBeenCalledWith({ ...localProfile, runtimeId: undefined }, false)); f.dispose();
});
...
it('creates SSH profiles without requiring Local’s shared URL registry', async () => {
  const f = fixture(); fireEvent.click(screen.getByRole('button', { name: 'SSH', exact: true }));
  fireEvent.change(screen.getByRole('textbox', { name: 'Name', exact: true }), { target: { value: 'Remote' } });
  fireEvent.change(screen.getByRole('textbox', { name: 'SSH host or alias' }), { target: { value: 'my-server' } });
  fireEvent.click(screen.getByRole('button', { name: 'Save and connect' }));
  await waitFor(() => expect(f.save).toHaveBeenCalledWith(expect.objectContaining({ id: expect.stringMatching(/^ssh:/), label: 'Remote', target: { kind: 'ssh', host: 'my-server' } }), false)); f.dispose();
});
...
it('does not close a reopened form when an old native request finishes', async () => {
  ...
  fireEvent.click(screen.getByRole('button', { name: 'Save and connect' })); f.reopen();
