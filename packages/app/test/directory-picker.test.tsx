...
function fixture() {
  ...
  const client = { getSnapshot: () => ({ info: { runtime_id: 'local' } }), host: { pickDirectory, directories } } as unknown as WhipClient;
...
it('ignores a native picker response after the target client changes', async () => {
  ...
  const remote = { ...f.client, getSnapshot: () => ({ info: { runtime_id: 'remote' } }) } as unknown as WhipClient;
