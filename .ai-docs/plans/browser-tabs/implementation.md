...
| 7 — packaged acceptance/docs/rollout | Partially exercised; environment-blocked and release gated | Fresh signed shipping-fuse Beta: default-OFF entry point/exit observed and native SSH confirmation shown. Enabled remote page loading and graceful quit/restart remain unverified; OS sample found a macOS keychain authorization wait. All owned local/remote fixtures cleaned up with evidence retained; see [root integration evidence](evidence-integration.md). Dependency audit remains unresolved. Packaged Browser default remains off |
