...
| 5 — SSH preview environments | Selected-host native gates pass; human entry implemented | Remote marker, Mac decoys, approved-port expansion, master-loss fail-closed and reconnect exercised; human entry still needs packaged validation |
...
| 7 — packaged acceptance/docs/rollout | Local enabled acceptance passes; release gated | Signed shipping-fuse Beta: default-OFF entry/exit and native SSH confirmation observed; user-consented normal-HOME run now passes local loading/input/navigation, enabled graceful quit/restart and tab/profile restore. Packaged SSH loading/recovery, agent control and remaining manual/performance gates remain open. All owned local/remote fixtures cleaned up with evidence retained; see [root integration evidence](evidence-integration.md). Earlier keychain wait's exact cause and dependency audit remain unresolved. Packaged Browser default remains off |
