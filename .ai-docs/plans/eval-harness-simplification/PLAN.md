...
- 2026-09-14: Phases A-E implemented in one change set on `compaction-loop-and-ui-cleanup`. Deleted `integrity.py` and `result_evidence.py`; rewrote `modal_cloud.py` (name+tags identity, marker-then-exit, no campaign cancel except a human), `modal_worker.py` (no admission/ACK waits, resource-fit preflight, marker last), `modal_cli.py` (public `read_file` fetch with a thread pool and retries, `prune`, fetch prunes by default). Configuration moved to `evals/frontier/modal.json`; `--settings`, `--fixture`, and `reconcile` removed. Adapter reads evidence from the bind-mounted `agent/whip/` directory and no longer downloads anything; dependency bootstrap is 600 s x 3 with receipts. Report: `provider_error` attribution, statuses `complete|cancelled|incomplete` with `status_detail`, known cost plus unknown-usage call count, cross-checks removed. Observer no longer writes `events.ndjson`. Whip (`internal/llm/openai.go`): a stream that fails before its first delta is repeated once. Tests: 74 Python tests (2,206 -> 1,697 lines), one Go table test. Verification and the campaign follow below.
- 2026-09-14 verification: 74 Python tests green on kuzco-4090; `internal/llm` Go tests green (the branch's `TestQueuedMailWakesIdleRootWithoutHumanInput` failure in `internal/daemon` predates this work). `doctor --integration --ref f99af76ff`: 8/8 fake-provider fixtures passed. Coordinator redeployed (controller digest e04f888d...). Real smoke `modal-smoke-20260914-s1` (8 tasks, jobs 8, `kimi-k3-fast`, ref f99af76ff): status `complete`, 6/8 passed, 8/8 evidence complete, 7/8 accounting complete, 0 provider errors, $18.30 known cost, 15 min wall, fetch 86 MB (2,021 files) then pruned. Cancel test `modal-cancel-20260914-c1` (jobs 1): `cancel` issued while the first VM ran; cancel delivered, worker exported and exited 0 within a minute, coordinator `cancelled`, 8/8 recorded, fetch status `cancelled` with the trial attributed `user_cancelled`, pruned. Decision 10 executed: 27 historical `evals/reports/*` directories removed from git and `evals/artifacts/*` (23 GB) deleted on the box; no accepted-baseline pointer existed.
- 2026-09-14 campaign `modal-full-20260914-b` (30 x 3, jobs 90, `kimi-k3-fast`, whip ref da4cc28d6, 19:56-20:47 UTC, 51 min at 90 concurrent): status `complete`; 53/90 verified passes (terminal-bench 45/63, datacurve 8/27; repetitions 18, 16, 19 of 30); 90/90 graded, 90/90 evidence complete, 83/90 accounting complete (7 attempts each carry one call with unknown usage); known model cost $675.73 (datacurve $606.42, terminal-bench $69.31); termination: 85 none, 3 `benchmark_deadline`, 2 `provider_error` (whip reported `context deadline exceeded`); 0 cancelled, 0 harness error codes, 0 unproven VM exits. Reports committed as `a0989f7`. Finding: the per-file SDK fetch took 46 min for 2.1 GB / 32,380 files (about 12 files/s); fetch now shells out to `modal volume get`, which moved the previous campaign's 4.9 GB in 3.5 min. Compared with `modal-full-20260914-a` (54/90 with 10 ungraded and 9 cancelled, $420.67 known but incomplete), this run graded every attempt; the higher known cost is mostly the 9 repository attempts that were cancelled last time and completed this time.
- 2026-09-15 second pass (`a41fa091`, from a four-angle review: reuse, simplification, efficiency, altitude; net -230 lines): the observer snapshots only when `PRAGMA data_version` moves (was every 250 ms), `state.json` carries only what the report reads, `aggregate` lost the pricing cross-check; the adapter's 2 s probe loop and `metrics.interim.json` are gone and `normalize_trial` keeps the observer's last `metrics.json` as the cost lower bound when a job has no native result (both backends); finality is one error code and accounting completeness is computed from the evidence files alone; seven unread row fields removed; `normalize_or_error` replaces three divergent fallbacks; cancellation is one Event set only by a human; bundle verified once at creation with a `{name: sha256}` inventory, `extract_bundle` deleted, one manifest and one `schedule.json`; the worker no longer re-verifies the bundle or writes `normalized.json`/`phase.json`; fetch normalizes against the fetch root and prune iterates keys; `tree_files` skips symlinks; `modal logs` removed. Verification: 76 tests green on the box; doctor integration 8/8 with the trimmed `state.json` (17 KB, seven keys) and no interim metrics file; Modal smoke `modal-smoke-20260915-simplify` (8 tasks, jobs 8, ref `a41fa091`) complete, 6/8 passed, 8/8 graded and evidence and accounting complete, $22.79 known and exact, 1,390 spans with none open, fetched through the new root-relative path (3,821 files) and pruned.
