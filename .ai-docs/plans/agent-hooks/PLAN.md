...
Commits on `feature/agent-definition`, in step order: definition and protocol
6.4; the executor registry serving hooks; `before_tool` at the host boundary;
`before_spawn` in `spawnAttempt`; `turn_start` in `RunTurn`; the SDK; this
documentation. `go test ./...`, `go vet ./...`, the repository analyzer,
`task contract`, and `task sdk` were green at every step.

Deviations from the plan as written:

- **`Events.EphemeralNotices` is a callback, as recorded in step 3.** The
  agent loop takes `Events` by value, so mid-turn notices are read through a
  function on every provider request rather than a mutated string.
- **Hook invocation ids are generated by the registry** (`hook-` plus a
  random token) rather than being ledger operation ids; a hook is not a
  ledgered operation, and a denied call never reaches the ledger.
- **`turn_start` skips are reported as `skipped`** with the cause, including a
  deny reply, which the hook cannot express; a deny from `turn_start` is
  ignored rather than emitted.
- **The spawn request keeps `mcp_tools` as raw JSON** so a rewrite passes it
  through unchanged; the plan's request shape did not list it.
- **Notice subject for turn hooks reads "this turn"**, since there is no
  operation to name.
- **The TUI shows denials and skips as notices** and lets rewrites ride with
  the cell; the plan left presentation to the REPL panel.
- **`stream.hook.decision` for allow emits nothing**, as planned, and the
  fixture-level SDK test orders replies by invocation id because the
  missing-handler reply posts before any awaited handler.
