...
class WhipAdapter:
    def __init__(self, *args, engine="starlark", binary=None, timeout=600,
                 max_cost=2.5, max_tokens=120000, max_turns=30, max_output=8192, commit=False, fixture=False,
                 native_defaults=False, **kwargs):
        ...
        try:
            ...
            try:
                async with asyncio.timeout(CLEANUP_TIMEOUT_SECONDS):
                    ...
                    for name in ("metrics.json", "outcome.json", "state.json", "events.ndjson", "identity.json", "configuration.json", "provider-catalog.json", "cli.ndjson", "cli.stderr", "sessions.db"):
                        try:
