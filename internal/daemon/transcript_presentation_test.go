...
func TestHostDurationRetainsSubMillisecondPrecision(t *testing.T) {
	node := &AgentSession{turn: turnJournal{TurnID: "turn"}}
	var emitted StreamEvent
	node.emit = func(_ string, event StreamEvent) { emitted = event }
	node.emitHostEvent("stream.cell.host", rlm.HostCall{Status: "completed", Duration: 125 * time.Microsecond})
	if emitted.Text != "125µs" {
		t.Fatalf("duration lost precision: %q", emitted.Text)
	}
}
