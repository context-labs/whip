...
func clientCLIParameters(operation, text string) (string, any, error) {
	...
	case "mcp":
		...
		if fields[0] == "import" {
			...
			if len(fields) != 3 || fields[1] != "claude" && fields[1] != "codex" && fields[1] != "project" && fields[1] != "opencode" || fields[2] != "on" && fields[2] != "off" {
				return fail("mcp import requires claude|codex|project|opencode and on|off")
			}
