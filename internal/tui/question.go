...
// page. Untouched pages are skipped when the batch is submitted.
...
func (m *model) submitQuestionBatch() (tea.Model, tea.Cmd) {
	...
	for i, set := range sets {
		...
		answer := draftAnswer(set, q.drafts[i])
		answers[i] = protocol.QuestionAnswerEntry{Answer: answer, Dismissed: len(answer) == 0}
	}
