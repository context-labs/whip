...
func TestOperationsMapCapabilitiesInCanonicalOrder(t *testing.T) {
	...
	if !slices.Equal(files, []string{"read"}) || !slices.Equal(shell, []string{"bash", "shell_start", "browser_exec", "workspace_process", "browser.open", "browser.attach", "browser.run", "browser.detach", "browser.allow_preview_port"}) || mcp {
