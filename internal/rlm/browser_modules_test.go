...
func TestBrowserLifecycleAndExplicitSpawnSurfaceBothEngines(t *testing.T) {
	for _, tc := range []struct{ engine, code string }{
		{EngineStarlark, `browser.list_tabs()
tab = browser.open(url="https://example.test")
...
browser.run(session="legacy", code="info()")`},
		{EngineQuickJS, `await browser.list_tabs({});
const tab = await browser.open({url:"https://example.test"});
