// Package theme owns renderer-independent theme definitions, color resolution,
// and the catalog shared by terminal and browser clients. It never stores the
// selected theme; presentation preferences belong to each client.
package theme
...
// Embedded returns the embedded themes, sorted by name. Errors are only
...
func parseSpec(data []byte, file string) (Spec, error) {
	if data = []byte(strings.TrimSpace(string(data))); len(data) == 0 || data[0] != '{' {
		return Spec{}, fmt.Errorf("theme %s: expected a JSON object", file)
	}
	...
	if err := dec.Decode(new(any)); err != io.EOF {
		return Spec{}, fmt.Errorf("theme %s: expected exactly one JSON object", file)
	}
	if spec.Name == "" {
...
func (s *Spec) Validate() error {
	...
	for _, block := range []struct {
		...
		for i := range sv.NumField() {
			if v := sv.Field(i).String(); v != "" && !validColor(v) {
