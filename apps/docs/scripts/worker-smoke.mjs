...
for (const doc of docs) assert(sitemap.includes(`https://inference.net${base}/docs/${doc.path}`))
...
try {
  for (const javaScriptEnabled of [true, false]) {
    ...
    for (const doc of docs) {
      ...
      assert.equal(await page.locator('link[rel="canonical"]').getAttribute('href'), `https://inference.net${base}/docs/${doc.path}`)
    ...
    if (javaScriptEnabled) {
      ...
      await page.getByRole('dialog').getByRole('link', { name: 'Quickstart', exact: true }).click()
