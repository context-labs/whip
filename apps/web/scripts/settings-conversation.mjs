...
for (const engine of engines) {
  await scenario(engine, 'repl', async ({ fixture, page, client, root, frames, replies, checks, workspace, panel, chat, notebook, tab, action, settings, back, density, select, stableAnchor, exactReturn, contentReads }) => {
    ...
    await action(root, 'Open REPL'); await notebook(root).waitFor();
    await select(panel(root), 'REPL agent', 'repl-child');
    await exactReturn('child REPL', () => notebook(root).waitFor());
    await expect(panel(root).getByRole('combobox', { name: 'REPL agent', exact: true })).toHaveText('repl-child');
    ...
    await select(panel(root), 'REPL agent', 'Root agent');
    await action(root, 'Open chat'); await panel(root).getByLabel('Message WHIP', { exact: true }).waitFor();
