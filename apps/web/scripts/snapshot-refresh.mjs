...
try {
  for (const name of (process.env.WHIP_WEB_BROWSERS ?? 'chromium,firefox').split(',')) {
    ...
    try {
      ...
      await expect(activity.locator('[data-activity-content]')).toContainText('2 executions');
      ...
      await expect(cells.nth(1).getByRole('region', { name: 'Output', exact: true })).toHaveText('first\nsecond');
