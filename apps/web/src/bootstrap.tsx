...
export function mountApplication(platform: AppPlatform, desktop?: DesktopBridge) {
  ...
  const sessionNavigator = createSessionNavigator(application.runtime, path => application.router.history.push(path));
  ...
  const initial = application.runtime.getSnapshot();
  if (!initial.needsConnectionSelection) {
    void application.runtime.connect().then(() => {
      if (!disposed && initial.error) application.runtime.report(initial.error);
    }).catch(() => {});
  }
