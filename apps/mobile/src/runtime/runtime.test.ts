...
test('a saved runtime mismatch closes before attachment or recovery and retains local work', async () => {
  const f = await fixture();
  const draft = f.runtime.setDraft('existing-draft', 'Keep this draft');
  await until(() => f.runtime.draftStatus('existing-draft') === 'saved');
  await expect(f.runtime.connect({ ...f.host, runtimeId: 'previous-runtime' })).rejects.toMatchObject({ kind: 'runtime_changed' });
  expect(f.requests().map(request => request.method)).toEqual(['initialize']);
  expect(f.peers[0].closed).toBe(true);
  expect(f.runtime.getSnapshot()).toMatchObject({ connecting: false, ready: false, client: undefined });
  expect(await f.storage.get('hosts', f.host.id)).toMatchObject({ runtimeId: 'previous-runtime', clientId: 'phone' });
  expect(f.runtime.draft('existing-draft')).toEqual(draft);
});
