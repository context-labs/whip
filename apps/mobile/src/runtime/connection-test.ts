...
export async function testConnection(input: string, options: {
  ...
  try {
    ...
    await step('websocket', async signal => {
      client = createWhipClient({ endpoint: origin, clientId: Crypto.randomUUID(), clientKind: 'automation',
        buildId: '@whip/mobile:connection-test', connectTimeoutMs: 15_000, queryTimeoutMs: 15_000, randomUUID: Crypto.randomUUID });
      await client.connect({ signal });
      if (options.expectedRuntimeId && client.requireConnected().runtime_id !== options.expectedRuntimeId)
        throw new WhipError('runtime_changed', 'The saved runtime identity does not match this daemon.');
    });
