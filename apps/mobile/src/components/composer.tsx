...
export function Composer({ value, onChangeText, onSend, disabled, offline, model, onOptions, onStop }: { value: string; onChangeText(text: string): void; onSend(): void; disabled: boolean; offline: boolean; model: string; onOptions(): void; onStop?: () => void }) {
  ...
  return <View style={{ borderRadius: tokens.radius.composer, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.panel, padding: 12, gap: 8 }}>
    <TextInput accessibilityLabel="Message" testID="message-input" value={value} onChangeText={onChangeText} multiline autoCapitalize="none" autoCorrect={false} placeholder={offline ? 'Draft while disconnected…' : 'Follow up…'} keyboardAppearance={dark ? 'dark' : 'light'} placeholderTextColor={colors.muted} selectionColor={colors.primary} textAlignVertical="top" style={{ minHeight: 48, maxHeight: 160, paddingHorizontal: 4, paddingVertical: 8, color: colors.foreground, fontFamily: display.regular, fontSize: 16 * display.textScale!, lineHeight: 24 * display.textScale! }} />
