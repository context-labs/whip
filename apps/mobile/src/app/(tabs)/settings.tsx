...
export default function SettingsScreen() {
  ...
  </Stack><Stack><Label style={{ fontSize: 22, fontWeight: '600' }}>Appearance</Label>
    {(['system', 'light', 'dark'] as const).map(mode => <RowButton key={mode} title={mode === 'system' ? 'Follow device appearance' : `${mode[0].toUpperCase()}${mode.slice(1)}`} selected={state.appearance.mode === mode} onPress={() => changeAppearance({ ...state.appearance, mode })} />)}
    <RowButton title="Light theme" detail={themeCatalog.find(t => t.id === state.appearance.light)?.name ?? 'Default light'} onPress={() => setChoosing(choosing === 'light' ? undefined : 'light')} />
    <RowButton title="Dark theme" detail={themeCatalog.find(t => t.id === state.appearance.dark)?.name ?? 'Default dark'} onPress={() => setChoosing(choosing === 'dark' ? undefined : 'dark')} />
    {choosing && <Stack>{themeCatalog.filter(t => t.dark === (choosing === 'dark')).map(t => <RowButton key={t.id} title={t.name} selected={state.appearance[choosing] === t.id} onPress={() => { changeAppearance({ ...state.appearance, [choosing]: t.id }); setChoosing(undefined); }} />)}</Stack>}
    <Label muted>Current theme: {theme.name}. Appearance is saved on this phone.</Label>
  </Stack><Stack><Label style={{ fontSize: 22, fontWeight: '600' }}>Command activity</Label>
