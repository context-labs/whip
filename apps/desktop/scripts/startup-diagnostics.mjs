...
export function startupDiagnostic({ completed, evidence = {}, archiveSHA256, results = [] }) {
  ...
  const diagnostic = {
    ...
    lastAttempt: {
      scenario: choice(last.scenario, ['first-launch', 'prepare', 'prepare-session', 'warm-attach', 'retained-start', 'idle', 'new-session-onboarding']),
      warmup: flag(last.warmup), state: choice(last.state, states),
      probeState: choice(probe.state, states), target: choice(probe.target, ['home', 'empty-frontdoor', 'new-session-onboarding', 'retained-session']),
      ...
      checks: Object.fromEntries(['host', 'noNotice', 'home', 'frontdoor', 'sdkConnected', 'appVisible', 'newSessionEnabled', 'session', 'transcript', 'visible', 'fonts', 'painted'].map(key => [key, flag(probe.checks?.[key])])),
      observation: {
        routeKind: choice(probe.observation?.routeKind, ['root', 'new-draft', 'retained-session', 'other']),
        uiVariant: choice(probe.observation?.uiVariant, ['empty-frontdoor', 'missing', 'provider-setup', 'composer', 'unknown']),
        sdkState: choice(probe.observation?.sdkState, ['connecting', 'connected', 'reconnecting', 'incompatible', 'paused', 'closed', 'unverified', 'unknown']),
        startupPhase: choice(probe.observation?.startupPhase, ['pending', 'exiting', 'entering', 'visible', 'unknown']),
      },
