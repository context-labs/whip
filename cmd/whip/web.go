...
func webCLI(args []string) error {
	flags := flag.NewFlagSet("whip web", flag.ContinueOnError)
	...
	if flags.NArg() != 0 {
		return errors.New("usage: whip web [--no-open] [--url https://whip.example]")
	}
	...
	if !*noOpen && !openWebBrowser(endpoint+"/") {
		fmt.Fprintln(os.Stderr, "whip: could not open the browser; open the URL above")
	}
	return nil
}
...
func webEndpoint(status daemonStatus, publishedURL string) (string, error) {
	if status.State == "stopped" {
		return "", errors.New("daemon is stopped; run `WHIP_NETWORK=1 whip daemon start`, then `whip web`")
	}
	if status.State != "running" {
		return "", fmt.Errorf("daemon is unavailable: %s; inspect `whip daemon status` and `whip daemon logs`; "+
			"replace it explicitly with `WHIP_NETWORK=1 whip daemon restart`", status.Error)
	}
	if status.NetworkEndpoint == "" {
		return "", errors.New("the running daemon has networking disabled; enable it explicitly with " +
			"`WHIP_NETWORK=1 whip daemon restart`, then `whip web` (restart interrupts active work)")
	}
	...
	if ip := net.ParseIP(parsed.Hostname()); ip != nil && ip.IsUnspecified() {
		return "", errors.New("the daemon is bound to a wildcard address; use `whip web --url http://HOST:PORT` " +
...
func checkWebAssets(ctx context.Context, endpoint string) error {
	...
	if response.StatusCode != http.StatusOK {
		if response.StatusCode == http.StatusForbidden {
			return errors.New("web host is not allowed; configure the exact WHIP_ALLOWED_HOSTS value and explicitly restart the daemon")
		}
		return fmt.Errorf("daemon does not expose the web app (HTTP %d); build with `npm ci && task build`, "+
	...
	if info.ProtocolMajor != daemon.ProtocolMajor || info.WebSocketPath != "/api/v3/ws" || info.ContentPath != "/api/v3/content/" {
		return errors.New("daemon web protocol is incompatible; update/build WHIP and explicitly restart the daemon")
	}
	if !info.Available {
		return errors.New("this daemon was built without web assets; run `npm ci && task build`, " +
