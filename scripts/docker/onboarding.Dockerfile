...
RUN --mount=type=cache,target=/go/pkg/mod --mount=type=cache,target=/root/.cache/go-build \
    CGO_ENABLED=0 go build -trimpath -ldflags "-X main.version=$WHIP_BUILD_VERSION" -o /out/whipcode ./cmd/whip
...
COPY --from=build /out/whipcode /usr/local/bin/whipcode
...
ENV HOME=/home/whip SHELL=/bin/bash LANG=C.UTF-8 \
    WHIPCODE_HOME=/home/whip/.whipcode \
    WHIPCODE_NETWORK=1 WHIPCODE_LISTEN=0.0.0.0:4000 \
    WHIPCODE_ALLOWED_HOSTS=localhost:4000,127.0.0.1:4000 \
    WHIPCODE_ALLOWED_ORIGINS=http://localhost:4000,http://127.0.0.1:4000
USER whip
WORKDIR /workspace
RUN git init --quiet \
